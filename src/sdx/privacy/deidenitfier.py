...
_ANALYZER_CACHE: AnalyzerEngine | None = None


def _allow_degraded() -> bool:
    """Return True unless the env var explicitly forbids degradation."""
    raw = os.getenv('PRESIDIO_ALLOW_DEGRADED')
    if raw is None:
        return True
    val = raw.strip().lower()
    if not val:
...
class Deidentifier:
    ...
    def _create_analyzer(self) -> AnalyzerEngine:
        """Create AnalyzerEngine with spaCy model selection and fallbacks.

        Returns
        -------
        AnalyzerEngine
            Configured analyzer engine with fallback support.

        Raises
        ------
        RuntimeError
            If spaCy models fail to load and degraded mode is not allowed.
        """
            ...
            try:
                provider = NlpEngineProvider(
                    nlp_configuration={
                        'nlp_engine_name': 'spacy',
                        'models': [
                            {'lang_code': 'en', 'model_name': alt_model}
                        ],
                    }
                )
                nlp_engine = provider.create_engine()
                ...
                if not _allow_degraded():
                    raise RuntimeError(
                        ...
                        'degrade PII detection. '
                        'Set PRESIDIO_ALLOW_DEGRADED=true (or omit it) to '
                        'allow fallback without spaCy.'
                    ) from e2
                logger.error(
                    'PRESIDIO_ALLOW_DEGRADED is true. Proceeding without '
                    'spaCy. PII detection quality will be significantly '
                    'reduced.'
                )
                try:
                    ...
                    raise RuntimeError(
                        'Presidio AnalyzerEngine failed to initialize without '
                        'spaCy.'
                    ) from e3
